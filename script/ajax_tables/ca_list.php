...
$csrf = $utility->csrf_generate('delete_ca.php', 600);

$table = <<<EOT
(
SELECT * FROM continuous_assessments
) temp
EOT;
...
$e = $csrf['token'];
$f = $csrf['formId'];
$l = 'Delete Continuous Assessments?';
$y = 'Yes';
$n = 'No';

return "
<button onclick='get_ca($d);' data-pc-animate='3d-rotate-InLeft' type='button' class='btn btn-primary btn-sm' data-bs-toggle='modal' data-bs-target='#modal-edit'><i class='fa fa-edit'></i></button>
<button onclick='drop(".htmlspecialchars_decode('&#039')."app/routes/delete_ca?id=$d&csrf_token=$e&csrf_form_id=$f".htmlspecialchars_decode('&#039').",".htmlspecialchars_decode('&#039')."$l".htmlspecialchars_decode('&#039').",
