...
unset($_SESSION['current_image_edit']);
...
<title><?php echo $info['name'].' - Edit Student'; ?></title>
...
<div class="content">
<form enctype="multipart/form-data" class="app_frm" action="app/routes/update_student" method="POST" autocomplete="off">
<div class="row">
...
<center><img id="image_preview" class="img_student mb-5 pre_capture_frame"

src="storage/images/students/<?php echo empty($std_profile['display_img']) ? $std_profile['gender'].'.png' : $std_profile['display_img']; ?>"
...
<div class="col-sm-12">
<input name="first_name" required type="text" value="<?php echo $std_profile['first_name']; ?>" placeholder="Enter first name" class="form-control txt_cap">
</div>
</div>
</div>
<div class="col-6">
...
<div class="col-sm-12">
<input name="last_name" required type="text" value="<?php echo $std_profile['last_name']; ?>" placeholder="Enter last name" class="form-control txt_cap">
</div>
</div>
</div>
</div>
...
<option <?php echo $std_profile['gender'] == 'Male' ? ' selected ' : ''; ?> value="Male">Male</option>
<option <?php echo $std_profile['gender'] == 'Female' ? ' selected ' : ''; ?> value="Female">Female</option>
</select>
</div>
</div>
</div>
...
<div class="col-sm-12">
<input name="email" required type="email" value="<?php echo $std_profile['email']; ?>" placeholder="Enter email address" class="form-control ">
</div>
</div>
</div>
<div class="col-6">
...
<option <?php echo $std_profile['account_status'] == 'ENABLED' ? ' selected ' : ''; ?> value="ENABLED">ENABLED</option>
<option <?php echo $std_profile['account_status'] == 'DISABLED' ? ' selected ' : ''; ?> value="DISABLED">DISABLED</option>
</select>
</div>
</div>
</div>
...
?><option <?php echo $std_profile['class'] == $list['id'] ? ' selected ' : ''; ?> value="<?php echo $list['id']; ?>"><?php echo $list['name']; ?></option><?php
}
?>
</select>
</div>
</div>
<input type="hidden" name="id" value="<?php echo $std_profile['reg_no']; ?>">
<input type="hidden" name="old_file" value="<?php echo $std_profile['display_img']; ?>">
<button type="submit" class="btn  btn-primary app_btn">Register Student</button>
</div>
</div>
</div>
<?= $utility->csrf_field('update_student.php', 600) ?>
</form>
</div>
</div>
...
if (!file) {
preview.src = 'storage/images/students/<?php echo empty($std_profile['display_img']) ? $std_profile['gender'].'.png' : $std_profile['display_img']; ?>';
return;
}
...
type: "POST",
dataType: "json",
url: "getter/capture_image_edit.php",
